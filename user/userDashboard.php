...
if (isset($_SESSION['user_id']) && isset($_SESSION['email'])) {
  ...
  $list_noticias = $noticia_obj->noticiasRecentes($_SESSION['user_id']);
} else {
  header('Location: ../../logout.php');
  exit;
}
...
<body>
  ...
  <div>
    ...
    <div id="noticias" class="flex-1 ml-64 p-2" style="display: flex; flex-direction: row; margin: 5px; flex-wrap: wrap; justify-content:center;">
    ...
    <div id="noticias" class="bg-white" style="display:flex; justify-content: center; grid-column: 2; margin-left:5px;">
      <?php foreach ($list_noticias as $noticia) : ?>
        ...
        <div class="m-10 max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
          ...
          <div class="p-5">
            <a href="#">
              <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white"><?= $noticia["nm_titulo"] ?></h5>
            </a>
            <a href="../noticia/detalhesNoticia.php?id=<?= $noticia['id_noticia'] ?>" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
              Saiba mais
